...
class VideoBaseModel(pl.LightningModule):
    ...
    def encode(self, x, discrete: bool = True):   #  输入:  b,t,c,h,w
        ...
        def _quantize_one(feat):
            ...
            return quant, emb_loss, info,bt

        if  not isinstance(h, list):
            h = [h]
        quant_list, emb_loss_list, info_list,bt_list = [], [], [],[]
        for h_i in h:
            q_i, e_i, info_i,bt_i = _quantize_one(h_i)
            quant_list.append(q_i)
            emb_loss_list.append(e_i)
            info_list.append(info_i)
            bt_list.append(bt_i)

        return quant_list, emb_loss_list, info_list,bt_list
    ...
    def forward(self, input, discrete=True):
        ...
        quant, diff, info, bt = self.encode(input,discrete=discrete)
        dec = self.decode(quant, bt=bt,discrete=discrete)
